...
mylastfm_two = mylastfm%>%
  ...
  mutate(pearson_corr =  cor(mediana_pop, news_prop_artist, method = "pearson"))
...
um_bootstrap <- function(x){
  pearson_corr = x %>% pull(pearson_corr)
  boot_x <- sample_n(pearson_corr,            # Amostre dos dados
                   size = NROW(pearson_corr), # Tamanho igual ao recebido
                   replace = TRUE)            # aqui é o bootstrap
  return(mean(boot_x))
}
